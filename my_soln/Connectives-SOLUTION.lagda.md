...
```
module plfa.part1.Connectives-SOLUTION where
```
...
```
-- The true definition of distributive law is defined before as
-- ×-distrib-⊎ : ∀ {A B C : Set} → (A ⊎ B) × C ≃ (A × C) ⊎ (B × C)
-- This implies ∀ {A B C : Set} → (A × C) ⊎ (B × C) → A ⊎ (B × C)
-- This can be easily proved
⊎-imply-× : ∀ {A B C : Set} → (A × C) ⊎ (B × C) → A ⊎ (B × C)
⊎-imply-× (inj₁(⟨ a , c ⟩)) = inj₁ a
⊎-imply-× (inj₂(⟨ b , c ⟩)) = (inj₂(⟨ b , c ⟩))
...
-- Counter-example : ⟨ (inj₁ a) , (inj₂ d) ⟩ can not deduce the corresponding (A × B) ⊎ (C × D)
--                   In other words, if there is only evidence for type A and D, it is NOT
--                   sufficient to deduce the left hand side.
```
