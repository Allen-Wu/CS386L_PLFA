...
```
module plfa.part1.Relations-SOLUTION where
```
...
```
-- Your code goes here
-- Lemma 1 : Monotonic on right-hand side
*-mono-r-≤ : ∀ (n p q : ℕ)
  → p ≤ q
  --------
  → n * p ≤ n * q
...
-- Lemma 2 : Monotonic on left-hand side
*-mono-l-≤ : ∀ (m n p : ℕ)
  → m ≤ n
  --------
  → m * p ≤ n * p
*-mono-l-≤ m n p m≤n
           rewrite *-comm m p | *-comm n p
         = *-mono-r-≤ p m n m≤n
-- Main proof
*-mono-≤ : ∀ (m n p q : ℕ)
  → m ≤ n
...
```
-- Your code goes here
-- Lemma 1 : Monotonic on the right
+-mono-r-< : ∀ (n p q : ℕ)
  → p < q
    ------
  → n + p < n + q
...
≤-imply-< : ∀ {m n : ℕ}
  ...
  → m < n
≤-imply-< (s≤s z≤n) = z<s
≤-imply-< {suc m} {suc n} (s≤s m≤n) = s<s (≤-imply-< {m} {n} m≤n)
-- The other way
<-imply-≤ : ∀ {m n : ℕ}
  → m < n
  --------
