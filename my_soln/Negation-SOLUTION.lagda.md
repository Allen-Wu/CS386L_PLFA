...
```
module plfa.part1.Negation-SOLUTION where
```
...
open import plfa.part1.Connectives using (⊤)
```
...
```
-- Your code goes here
-- Simple inversion lemma for strict unequality
inv-s<s : ∀ {m n : ℕ}
  → (suc m) < (suc n)
  -------------------
  → m < n
inv-s<s (s<s m<n) = m<n
-- Main proof of irreflexive
<-irreflexive : ∀ (n : ℕ) → ¬ (n < n)
<-irreflexive zero = λ ()
...
inv-s≡s : ∀ {m n : ℕ}
  ...
  → m ≡ n
inv-s≡s refl = refl
-- Exact one proof for m < n case
<-exact : ∀ (m n : ℕ) → m < n → (¬ m ≡ n) × (¬ n < m)
<-exact zero (suc n) m<n = (λ ()) Data.Product., (λ ())
<-exact (suc m) (suc n) sucm<sucn = (λ x → proj₁ (<-exact m n (inv-s<s sucm<sucn)) (inv-s≡s x)) Data.Product., (λ x → proj₂ (<-exact m n (inv-s<s sucm<sucn)) (inv-s<s x))
-- Exact one proof for m≡n case
≡-exact : ∀ (m n : ℕ) → m ≡ n → (¬ m < n) × (¬ n < m)
≡-exact zero zero z≡z = (λ ()) Data.Product., (λ ())
≡-exact (suc m) (suc n) sm≡sn = (λ x → proj₁ (≡-exact m n (inv-s≡s sm≡sn)) (inv-s<s x)) Data.Product., (λ x → proj₂ (≡-exact m n (inv-s≡s sm≡sn)) (inv-s<s x))
-- Exact one proof for n < m case
>-exact : ∀ (m n : ℕ) → n < m → (¬ m ≡ n) × (¬ m < n)
>-exact (suc m) zero n<m = (λ ()) Data.Product., (λ ())
>-exact (suc m) (suc n) sn<sm = (λ x → proj₁ (>-exact m n (inv-s<s sn<sm)) (inv-s≡s x)) Data.Product., (λ x → proj₂ (>-exact m n (inv-s<s sn<sm)) (inv-s<s x))
```
...
⊎-dual-× =
  record
    {
      to = λ{ x → (λ x₁ → x (inj₁ x₁)) Data.Product., (λ x₁ → x (inj₂ x₁)) }
    ; from = λ{ x → (λ {(inj₁ a) → (proj₁ x) a
                       ; (inj₂ b) → (proj₂ x) b}) }
    ; to∘from = λ y → refl
    ; from∘to = λ{ x → extensionality λ {(inj₁ a) → refl ; (inj₂ b) → refl}}
    }
```
...
Ans : I can not prove the isomorphism between ¬ (A × B) and (¬ A) ⊎ (¬ B)
      But I can prove the weak relation that (¬ A) ⊎ (¬ B) → ¬ (A × B)
```
⊎-implies-× : ∀ {A B : Set} → (¬ A) ⊎ (¬ B) → ¬ (A × B)
⊎-implies-× = λ {(inj₁ ¬a) → λ x → ¬a (proj₁ x)
               ; (inj₂ ¬b) → λ x → ¬b (proj₂ x)}
```
