...
```
module plfa.part1.Induction-SOLUTION where
```
...
```
-- Your code goes here
-- Lemma 1 : right hand side zero multiplication
...
-- Lemma 2 : Helper lemma
+-comm-suc : ∀ (m n : ℕ) → suc(m + n) ≡ suc(n + m)
+-comm-suc m n =
  begin
    suc(m + n)
  ≡⟨ cong suc (+-comm m n) ⟩
...
-- Lemma 3 : right hand side multiplication suc
*-suc-right : ∀ (m n : ℕ) → m * (suc n) ≡ m + m * n
*-suc-right zero n =
  begin
    zero * (suc n)
  ≡⟨⟩
...
```
-- Your code goes here
-- No proof required. Only pattern matching.
...
-- Your code goes here
{-
open import plaf.part1.Naturals using (Bin; inc; to; from)
open Bin
...
from-inc (b I)
  rewrite -- from (inc b) + (from (inc b) + zero)
    ...
    from-inc b -- suc (from b) + (suc (from b) + 0)
            -- ≡ suc 
-}
```
