...
-- Define ⇔-dec
_⇔-dec_ : ∀ {A B : Set} → Dec A → Dec B → Dec (A ⇔ B)
(yes x) ⇔-dec (yes y) = yes (record { to = λ x₁ → y ; from = λ x₁ → x })
(yes x) ⇔-dec (no ¬y) = no (λ x₁ → ¬y (_⇔_.to x₁ x))
(no ¬x) ⇔-dec (yes y) = no (λ x → ¬x (_⇔_.from x y))
(no ¬x) ⇔-dec (no ¬y) = yes (record { to = λ x → {!!} ; from = λ x → {!!} })
-- Main proof of iff-⇔
